...
exports.sendNotification = functions.database.ref('Messages/{roomId}/{messageId}').onCreate(event => 
    ...
    if (!dataVal) { 
     ...
     const sendProfile = dataVal.profileImg ? dataVal.profileImg : '';
     // 프로필 이미지
     ...
     return Promise
     .all([promiseRoomUserList, promiseUsersConnection])
     .then(results => 
        {
                         ...
                         if(value){ 
                            ...
                            return console.log('UserConnections Data가 없습니다'); 
                             ...
                             for(let i=0; i < arrTargetUserListLength; i++){ 
                                 ...
                                 admin.database()
                                 .ref(`FcmId/${arrTargetUserList[i]}`)
                                 .once('value',fcmSnapshot => { 
                                     console.log('FCM Token : ', fcmSnapshot.val()); 
                                     const token = fcmSnapshot.val(); 
                                     if(token){ const payload = { 
                                         notification: { 
                                             title: sendUserName, body: sendMsg, click_action :`http://개별Firebase앱주소/?roomId=${roomId}`, // icon: sendProfile 
                                            } }; 
                                         admin.messaging().sendToDevice(token, payload)
                                         .then(response => { response.results.forEach((result, index) => { 
                                             const error = result.error; if (error) { 
                                                 console.error('FCM 실패 :', error.code); 
                                                }else{ console.log('FCM 성공'); } }); }); } }); } }); });
